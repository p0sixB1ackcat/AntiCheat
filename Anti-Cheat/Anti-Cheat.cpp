...
)
{
 ...
 NTSTATUS DispatchIoControl(PDEVICE_OBJECT pDevicceObject, PIRP pIrp)
 {
     ...
     switch (dwControlCode)
     {
        ...
        break;
        case ACCTL_CODE_START_HOOK:
        {
            ntStatus = PsLookupProcessByProcessId(PsGetCurrentProcessId(), &pEprocess);
            if (!NT_SUCCESS(ntStatus))
            {
                KdPrint(("Get Process Fail:0x%x!\n", ntStatus));
            }
            __try
            {
                KeStackAttachProcess(pEprocess, &kApcState);
                Hook_KeUserModuleCallback((PVOID)&H_KeUserModuleCallback, &Original_KeUserModuleCallback);
            }
            __finally
            {
